...
describe("buildTrustedOrigins", () => {
  it("dev: allows any localhost port + vercel previews + BETTER_AUTH_URL", () => {
    const o = buildTrustedOrigins({ NODE_ENV: "development", BETTER_AUTH_URL: "http://localhost:3000" });
  ...
  it("production: does NOT trust localhost, keeps vercel previews + prod origin", () => {
    const o = buildTrustedOrigins({ NODE_ENV: "production", BETTER_AUTH_URL: "https://syncfit.example.com" });
  ...
  it("omits BETTER_AUTH_URL entry when unset; never empty of vercel previews", () => {
    const o = buildTrustedOrigins({ NODE_ENV: "production", BETTER_AUTH_URL: undefined });
    expect(o).toContain("https://*.vercel.app");
