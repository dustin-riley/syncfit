...
4. **Set the other env vars in Vercel:** `BETTER_AUTH_SECRET`, `ANTHROPIC_API_KEY`, and `BETTER_AUTH_URL` set to the deployed origin (e.g. `https://syncfit.vercel.app`). After the first deploy, update `BETTER_AUTH_URL` to the real assigned domain and redeploy.
