...
export async function runReadinessAnalysis(opts: {
  userId: string; now?: Date; generate?: GenerateFn;
}): Promise<AnalyzeOutcome> {
  const now = opts.now ?? new Date();
  const { date, dow } = todayInfo(now);

  const [planned] = await db.select().from(plannedSession)
    .where(and(eq(plannedSession.userId, opts.userId), eq(plannedSession.dayOfWeek, dow)));
  ...
  const rows = await db
    ...
    .where(and(eq(workoutSet.userId, opts.userId), gte(workout.performedAt, cutoff)));
  const setRows: SetRow[] = rows.map((r) => ({
    exerciseName: r.exerciseName, performedAt: r.performedAt,
    weight: Number(r.weight), reps: r.reps,
  }));
  ...
  try {
    ...
    await db.insert(readinessAnalysis).values({
      userId: opts.userId, analysisDate: date, planSnapshot: planned, loadSnapshot: load,
      verdict: result.verdict, headline: result.headline, rationale: result.rationale,
      modifications: result.modifications, model: MODEL_ID,
    });
    return { result };
  } catch (e: any) {
